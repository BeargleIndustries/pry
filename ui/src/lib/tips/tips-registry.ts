...
export const tipsRegistry: TipDef[] = [
  {
    ...
    placement: 'right',
    blurb:
      "Each cell is one attention head at one layer (12 layers \u00d7 12 heads, so 144 little specialists all doing slightly different things). Rows are layers, columns are heads, and brightness is how much attention this cell paid to the currently-focused token.",
    why:
  ...
  },
  {
    ...
    placement: 'bottom',
    blurb:
      "Same idea as Panel A's token strip, different job. Click a token and Panel B shows the SAE features that fired on that specific word.",
    why: null,
  },
  {
    ...
    placement: 'left',
    blurb:
      "Each row is one SAE feature that fired on the current token. The bar shows how strongly it activated, and the label tries to describe what the feature represents.",
    why:
      "Two things worth knowing. First: the labels are auto-generated from the training examples where the feature fires hardest (via Neuronpedia), which means they're a best guess, not a guarantee. Always peek at the example text underneath before trusting a label, because sometimes the auto-label catches one pattern while the feature is actually doing two or three things at once. Second: high activation doesn't mean 'the model is thinking about this concept hardest.' It means 'this concept is most present in the token's representation right now.' Subtle difference, but it matters when you're trying to figure out what the model was actually doing versus what was just sitting in its working memory.",
  },
  {
  ...
  },
  {
    ...
    placement: 'bottom',
    blurb:
      "Different layers see different things. Pick a layer to re-run the SAE at that depth and see which features fire.",
    why:
  ...
  },
  {
    ...
    why:
      "This is the standard way researchers find circuits. You pick two prompts that the model handles differently (a 'clean' prompt and a 'corrupted' one), run both, then systematically replace one component's activations from the clean run into the corrupted run. If swapping head 7.3's output from clean into corrupted makes the corrupted prompt behave like the clean one, that head is causally responsible for the difference. It's sort of like debugging by swapping parts until you find the one that was broken. The heatmap shows the impact of patching each component, so you can see the whole model's causal structure at a glance.",
  },
  {
    id: 'circuit-graph',
